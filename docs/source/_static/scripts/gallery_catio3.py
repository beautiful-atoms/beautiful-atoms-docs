...
cto.bond.settings[('Ti', 'O')].search = 0
cto.boundary = 0.01
cto.bond.settings.remove(('Ca', 'O'))
cto.polyhedra.settings['Ti'].color = [0.7, 0.4, 0.0, 1.0]
...
cto['Ca'].scale = 0.5
cto.boundary = 0.01
cto.bond.settings[('Ti', 'O')].style = '0'
cto.bond.settings[('Ti', 'O')].width = 0.005
cto.bond.settings[('Ti', 'O')].color1 = [0.1, 0.1, 0.1, 1.0]
cto.model_style = 1
...
# polyhedra
cto.bond.settings[('Ti', 'O')].search = 1
cto.bond.settings[('Ti', 'O')].width = 0.01
cto.polyhedra.settings['Ti'].color = [0.5, 0.0, 0.7, 0.2]
cto.boundary = 0.01
cto.model_style = 2
plane.location = [0, 0, cto.get_all_vertices()[:, 2].min() -max(cto.size) ]
...
cto.bond.settings[('Ti', 'O')].search = 1
cto.bond.settings[('Ti', 'O')].width = 0.01
cto.polyhedra.settings['Ti'].color = [0.2, 0.5, 0.7, 1.0]
cto.boundary = 0.01
cto.model_style = 2
plane.location = [0, 0, cto.get_all_vertices()[:, 2].min() - max(cto.size) ]
