...
The function ``species`` in :class:`Batom` object is used to specify the symbol of species. We store the ``species`` infomation in ``atoms.arrays['species']`` for ASE atoms.
